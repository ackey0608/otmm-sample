package com.example.participant2.controller;
...
public class TestController {
    ...
    public ResponseEntity<String> create(
        ...
        log.info("Participant2 start. LRA: {}", lraId);

        if ("error2nd".equals(pattern)) {
            log.error("error!");
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("NG");
        }

        log.info("Participant2 end. LRA: {}", lraId);
    ...
    public ResponseEntity<?> complete(
        ...
        log.info("Participant2 completed. LRA: {}", lraId);
    ...
    public ResponseEntity<?> compensate(
        ...
        log.warn("Participant2 cancelled. LRA: {}", lraId);
        return ResponseEntity.ok().build();
    }
